...
# Install Apache Ant and wget
RUN apt-get update && apt-get install -y ant wget && rm -rf /var/lib/apt/lists/*

# Download NetBeans CopyLibs JAR
RUN wget https://repo1.maven.org/maven2/org/netbeans/api/org-netbeans-modules-java-j2seproject-copylibstask/RELEASE130/org-netbeans-modules-java-j2seproject-copylibstask-RELEASE130.jar -O /opt/copylibs.jar

# Copy the project files
COPY . .

# Build the project using Ant (this creates dist/book_store_-1.war)
RUN ant -Dj2ee.server.home=/usr/local/tomcat -Dlibs.CopyLibs.classpath=/opt/copylibs.jar dist
